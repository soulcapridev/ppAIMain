...
//    List<Part_BAK> _partsInEvent = new List<Part_BAK>();
...
//        if (toiletDif > 0)
//        {
//            List<Part_BAK> outsideParts = _existingParts.Where(p => !_partsInEvent.Contains(p) && p.Type == PartType.Toilet).ToList();
...
//                Part_BAK outsidePart = outsideParts[i];
...
//        else if(toiletDif < 0)
//        {
//            List<Part_BAK> insideParts = _partsInEvent.Where(p => _partsInEvent.Contains(p) && p.Type == PartType.Toilet).ToList();
//            for (int i = 0; i < -toiletDif; i++)
//            {
//                Part_BAK insidePart = insideParts[i];
//                PPMoveTask t = new PPMoveTask();
//                t.tenant = _ai;
//                t.Part = insidePart;
//                //t.taskDay = _eventDate; STILL TO IMPLEMENT
